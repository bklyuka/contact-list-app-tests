...
class TestLogin:
    ...
    def test_login_with_with_valid_data(self, login_page: LoginPage):
    ...
    def test_login_without_email_and_password_set(self, login_page: LoginPage):
        login_page.submit_btn.click()
        expect(login_page.error).to_have_text(login_page.incorrect_password_error_msg)

    def test_login_without_password_set(self, login_page: LoginPage, credentials_data: LoginCredentials):
        login_page.email_field.fill(credentials_data.email)
        login_page.submit_btn.click()
        expect(login_page.error).to_have_text(login_page.incorrect_password_error_msg)

    def test_login_without_email_set(self, login_page: LoginPage, credentials_data: LoginCredentials):
